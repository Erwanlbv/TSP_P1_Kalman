...
def cart_to_pol(x, y): #Format 1*2
    return np.array([np.arctan(y/x), np.sqrt(x**2 + y**2)])
...
def filtre_kalman_radar(F, Q, R, y_k, x_kalm_prec, P_kalm_prec):
    ...
    #Partie filtrage :
    H = np.array([
        [-x_pred_suiv[2]/(x_pred_suiv[0]**2 + x_pred_suiv[2]**2), 0,
         x_pred_suiv[0]/(x_pred_suiv[0]**2 + x_pred_suiv[2]**2), 0],
        [x_pred_suiv[0] / np.sqrt(x_pred_suiv[0]**2 + x_pred_suiv[2]**2), 0,
         x_pred_suiv[2] / np.sqrt(x_pred_suiv[0]**2 + x_pred_suiv[2]**2), 0]
    ])
    ...
    y_k = y_k + H.dot(x_pred_suiv) - (cart_to_pol(x_pred_suiv[0], x_pred_suiv[2]))
